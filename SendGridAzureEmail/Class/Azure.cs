...
namespace SendGridAzureEmail.Class
{
	public class AzureClass
	{
		...
		public List<String> ListBlobFile(string PathBlob, string ContainerBlobName, string FolderIn = "")
		{
			...
			foreach (BlobItem blobItem in ListblobFiles)
			{
				...
				AccessTier PropAccessTier = (AccessTier)blobItem.Properties.AccessTier;
				long PropContentLength = blobItem.Properties.ContentLength.Value;
				if (PropAccessTier == AccessTier.Hot && PropContentLength>0)
				{
					listName.Add(BlobName);
				}
			}
